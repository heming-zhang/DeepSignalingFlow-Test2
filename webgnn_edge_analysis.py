...
class PanEdgeAnalyse():
    ...
    def reform_weight_adj(self, fold_n, model):
        ...
        if os.path.exists('./analysis_oneil/fold_' + str(fold_n) + '_pan') == False:
            os.mkdir('./analysis_oneil/fold_' + str(fold_n) + '_pan')
        np.save('./analysis_oneil/fold_' + str(fold_n) + '_pan/first_conv_up_weight.npy', first_conv_up_weight)
        np.save('./analysis_oneil/fold_' + str(fold_n) + '_pan/first_conv_down_weight.npy', first_conv_down_weight)
        np.save('./analysis_oneil/fold_' + str(fold_n) + '_pan/block_conv_up_weight.npy', block_conv_up_weight)
        np.save('./analysis_oneil/fold_' + str(fold_n) + '_pan/block_conv_down_weight.npy', block_conv_down_weight)
        np.save('./analysis_oneil/fold_' + str(fold_n) + '_pan/last_conv_up_weight.npy', last_conv_up_weight)
        np.save('./analysis_oneil/fold_' + str(fold_n) + '_pan/last_conv_down_weight.npy', last_conv_down_weight)
        ...
        # node_num_dict_df = pd.read_csv('./analysis_oneil/node_num_dict.csv')
        ...
        node_num_dict_df.to_csv('./analysis_oneil/node_num_dict.csv', header=True, index=False)
        ...
        kegg_gene_interaction_df.to_csv('./analysis_oneil/fold_' + str(fold_n) + '_pan/kegg_weighted_gene_interaction.csv', index=False, header=True)
...
if __name__ == "__main__":
    ...
    fold_n = 5
    load_path = './data/result/oneil_webgnn/epoch_200_4/best_train_model.pt'
    model.load_state_dict(torch.load(load_path, map_location=device))

    if os.path.exists('./analysis_oneil') == False:
        os.mkdir('./analysis_oneil')
